const anttTable = {
  ...
  "Perigosa (frigorificada ou aquecida)": {
    ...
    7: { CCD: 9.2967, CC: 1022.80 },
    9: { CCD: 10.4042, CC: 1092.20 },
  },
};
...
async function getCoordinates(cep) {
  try {
    const response = await fetch(`https://nominatim.openstreetmap.org/search?format=json&postalcode=${cep}&country=Brazil`);
    ...
    throw new Error(`Falha ao obter coordenadas para o CEP ${cep}: ${error.message}`);
  }
}
...
async function calculateMinFreight(formData, numAxles) {
  ...
  if (formData.load_description.features.perishable && cargoType !== "Carga Frigorificada" && cargoType !== "Produtos Farmacêuticos") {
    anttCargoType = "Frigorificada ou Aquecida";
  } else if (formData.load_description.features.perishable && (cargoType === "Químicos Perigosos" || cargoType === "Inflamáveis")) {
  ...
  try {
    ...
    if (!distanceData.routes || !distanceData.routes[0] || !distanceData.routes[0].distance) {
    ...
    if (anttTable[anttCargoType] && anttTable[anttCargoType][numAxles]) {
      ...
      throw new Error(`Tipo de carga '${anttCargoType}' ou número de eixos ${numAxles} não encontrado na tabela da ANTT.`);
    }
  } catch (error) {
